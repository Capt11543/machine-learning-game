...
print_map = [[0 for x in range(width) for y in range(height)]]#Multidimensional list for when map is printed
rooms = 0#Room count
...
while not has_boss:#Run as long as there is no boss
    for index_y in dungeon_map:#Y loop
        for index_x in dungeon_map[index_y]:#X loop
...
#Now the problem is, how the fuck do I print this